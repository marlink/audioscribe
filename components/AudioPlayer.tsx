...
const AudioPlayer = forwardRef<AudioPlayerRef, AudioPlayerProps>(({ src, onTimeUpdate, onDurationChange, onEnded }, ref) => {
  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 p-4 shadow-sm w-full flex flex-col gap-3">
      <audio
        ref={audioRef}
        src={src}
      ...
      <div className="relative flex items-center group h-5 -mt-1">
        ...
        <div 
          className="absolute h-3 w-3 bg-blue-600 rounded-full shadow border border-white pointer-events-none transition-transform group-hover:scale-125"
      ...
      <div className="flex flex-wrap items-center justify-between gap-y-2">
        ...
        <div className="flex items-center gap-2 flex-1 min-w-[100px]">
            <button 
                onClick={cycleSpeed}
                className="text-xs font-bold text-gray-500 bg-gray-100 hover:bg-gray-200 hover:text-blue-600 px-2 py-1.5 rounded-md transition-colors w-12 text-center"
            ...
            <div className="group relative flex items-center">
                ...
                <div className="hidden group-hover:flex absolute bottom-full left-0 mb-2 p-2 bg-white shadow-lg border border-gray-100 rounded-lg w-32 items-center gap-2 z-30">
                     <input 
                     ...
                     />
                     <span className="text-xs font-mono text-gray-500 w-8 text-right">{Math.round((isMuted ? 0 : volume) * 100)}%</span>
                </div>
            </div>
        </div>
        ...
        <div className="flex items-center gap-1 justify-center flex-1 min-w-[200px]">
          ...
          <button onClick={togglePlay} className="p-3 bg-blue-600 text-white rounded-full hover:bg-blue-700 shadow-md transition-transform active-scale-95 mx-2" aria-label={isPlaying ? "Pause" : "Play"}>
            {isPlaying ? (
        ...
        <div className="flex-1 text-right min-w-[80px]">
            <div className="text-sm font-mono text-gray-500 select-none">
      ...
      <style>{`
        .control-btn {
          @apply p-2 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-full transition-colors;
        }
      `}</style>
    </div>
  );
